...
class Node(object):
	...
	def __init__(self, 
		...
		# if goal_cost != None:
		# 	self.total_cost = movement_cost + goal_cost
		# else:
		# 	self.total_cost = goal_cost
		# 	
	...
	def isDuplicate(self, random_node):
		if random_node.orientation is None:
			return (utils.heuristic_cost(self.current_coords, random_node.current_coords) < 0.5)
		
		return ((utils.heuristic_cost(self.current_coords, random_node.current_coords) < 0.5) and (abs(self.orientation - random_node.orientation) < 30))
