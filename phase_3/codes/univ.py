...
def function(exploration_node_vector, path_node_vector, goal_node):
	...
	for node in exploration_node_vector:
		...
		u -= x
		v -= y
		q = plt.quiver(x, y, u, v, units='xy', scale=1.1, width=0.2, headwidth=0.5, headlength=0.5, color=colors[color_i%len(colors)])
	...
	for node in path_node_vector[:-1]:
		...
		u -= x
		v -= y
		q = plt.quiver(x, y, u, v, units='xy', scale=1.1, width=0.2, headwidth=0.5, headlength=0.5, color="black")
